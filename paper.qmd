...
2. What Dataset-JSON is, where it came from, and why it matters.
...
XPT is not “bad”—it is widely supported and deeply embedded in regulatory processes. However, when viewed through a modern software lens, it can function like a legacy constraint:
...
- **Metadata and expressiveness constraints:** labels, formats, and rich metadata structures are difficult to manage consistently across tools and conversions.
...
Dataset-JSON is a CDISC standard intended to represent clinical tabular datasets using JSON. It exists in the context of increasing adoption of structured data standards and the need to support interoperability beyond a single vendor ecosystem.
...
Pilot 5 uses Dataset-JSON v1.1 (as referenced in the ADRG standards inventory) and demonstrates converting and producing these artifacts in R in a transparent manner.
...
Pilot 5 extends this by integrating Dataset-JSON as the dataset transport mechanism for the delivered datasets.
...
Pilot 5 objectives include delivering a publicly accessible R-based submission package using Dataset-JSON, and expanding prior pilot approaches by generating ADaM datasets using R.
...
Pilot 5 highlights the “real work” involved in making Dataset-JSON operational:

- **Metadata fidelity:** ensuring dataset and variable labeling is complete and consistent (e.g., ensuring dataset labels exist when reading source data).
- **Precision and numeric representation:** explicitly controlling serialization choices (e.g., `float_as_decimals = TRUE` in Dataset-JSON writing and downstream conversion considerations).
- **End-to-end reproducibility:** not only writing JSON, but also proving that outputs regenerate and match expectations.
